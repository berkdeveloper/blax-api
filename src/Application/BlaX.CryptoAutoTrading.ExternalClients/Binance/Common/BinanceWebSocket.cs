    ...
    public class BinanceWebSocket : IDisposable
    {
        ...
        public async Task ConnectAsync(CancellationToken cancellationToken)
        {
            if (this.handler.State != WebSocketState.Open)
            {
                ...
                await this.handler.ConnectAsync(url, cancellationToken);
                await Task.Factory.StartNew(() => ReceiveLoop(this.loopCancellationTokenSource.Token, this.receiveBufferSize), this.loopCancellationTokenSource.Token, TaskCreationOptions.LongRunning, TaskScheduler.Default);
            }
        }
